...
// optional. `details` renders as always-visible content below the
// description (Process steps never use it, so they render exactly as
// before).
      ...
      {items.map((item, index) => (
        <li key={item.label ?? index} className="flex gap-6">
          ...
          <div className="flex-1 pb-2">
            ...
            {item.details && <div className="mt-4 max-w-2xl">{item.details}</div>}
          </div>
        </li>
      ))}
    </ol>
